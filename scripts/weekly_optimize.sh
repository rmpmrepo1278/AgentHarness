    ...
    for dir in /opt/ik_llama /opt/Ollama; do
        if [ -d "${dir}/.git" ]; then
            ...
            if ! git fetch --dry-run &>/dev/null; then
                echo "- **${name}**: Fetch failed (network issue or repo unreachable)" >> "${WEEKLY_REPORT}"
                continue
            fi
            local behind
            behind=$(git status -uno | grep -c "is behind" || echo "0")

            if [ "${behind}" -gt 0 ]; then
                local latest
                latest=$(git ls-remote --heads origin main 2>/dev/null | cut -c1-7 || echo "unknown")
                echo "- **${name}**: Current ${current_hash}, remote has updates (${behind} commits behind). Consider rebuilding." >> "${WEEKLY_REPORT}"
            else
...
EOF
    ...
    # --- Phase 6: Sentinel Weekly Scorecard ---
    log_ok "Adding Sentinel weekly scorecard..."
    SENTINEL_SCORE=$(cd /home/rohit/.hermes/hermes-agent && \
        /home/rohit/.hermes/hermes-agent/.venv/bin/python3 -c "
import sys, json
sys.path.insert(0, '.')
from plugins.sentinel.reporting import generate_weekly_scorecard
print(generate_weekly_scorecard())
" 2>/dev/null || echo "\n⚠️ Sentinel scorecard unavailable.")
    echo "${SENTINEL_SCORE}" >> "${WEEKLY_REPORT}"
